    ...
    if(id) {
        if(widgets && widgets.data){
                ...
                if(widData && widData.length>0){
                    // widData.widgets=[];
                    //widData.widgets=[].concat(reqData.widgets);
                      console.log("widgets1:",widData.widgets);
                   widgets.remove(widData);
                    db.saveDatabase();
                    var widgets1 = db.getCollection('steps') || db.addCollection('steps');
                    var tempId =widData.id ||0;
                   reqData.id = tempId+1;
                    widgets1.insert(reqData);
                   // reqData.id = reqData.id ||id;
