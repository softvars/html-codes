...
//var RestRegex = new RegExp("^(?:[\\w]*):([\\-\\d\\w\\.\\:]+)", "i"); RestRegex.exec('PNR:4207642135');
//var RestRegex = new RegExp("^(?:[^:]*):([\\-\\d\\w\\.\\:]+)", "i"); RestRegex.exec('PNR:4207642135');
...
function parseTkt(tkt) {
    ...
    if(!tkt_info_raw.length) {
        tktObj = null;
    } else if(tkt_info_raw.length && tkt_info_raw.length < 9){
        tktObj = null;
    }
    else {
